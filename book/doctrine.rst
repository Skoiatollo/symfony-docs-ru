...
.. note::
    ...
    нельзя смешивать определения метаданных YAML и определения через аннотациии
    в классе-сущности PHP.

.. tip::

    Имя таблицы необязательно и если его не указать, то оно будет определено
...
.. caution::
    ...
    Будьте осторожны имена классов и свойств не отображаются на защищённые
    ...
    лучше экранировать такие имена. С другой стороны, если вы свободны выбрать
    схему для вашей базу данных, просто отобразите ее на другое имя таблицы или колонки. 
    См документацию Doctrine: `Persistent classes`_ and `Property Mapping`_. 
...
.. note::
    ...
    Например, чтобы уберечь ``@fn`` аннотацию от вызова исключения, добавьте
    следующее::
...
Теперь, хотя Doctrine знает, как сохранить объект ``Product`` в базу данных,
...
.. caution::
     ...
     Имейте в виду, что генератор сущностных классов Doctrine создает простые
     геттеры и сеттеры. Вам следует проверить созданные сущностные классы и 
     приспособить логику геттеров и сеттеров к своим нуждам. 
    
.. sidebar:: Немного больше о ``doctrine:generate:entities``

    С помощью команды ``doctrine:generate:entities`` вы можете:

        * создавать геттеры и сеттеры;

        * создавать классы хранилищ данных (repository classes) с настроенной
          аннотацией  ``@ORM\Entity(repositoryClass="...")``;

        * создавать подходящий конструктор для связей for 1:n и n:m.

    Команда ``doctrine:generate:entities`` сохраняет резервные копии оригинала
    ``Product.php``, с именем ``Product.php~``. В некоторых случаях, присутствие 
    этого файла может вызвать ошибку Невозможно переопределить класс ("Cannot 
    redeclare class"). Его можно безопасно удалить. Можно также использовать
    опицю  ``--no-backup``, чтобы предотвратить создание этих резервных копий. 

    Учтите, что вам не так уж *обязательно* использовать эту команду. Doctrine 
    не основывается на создании кода. Как и в случае с обычными PHP классами, 
    вам просто нужно убедиться, что ваши protected/private свойства имеют
    свои методы геттера и сеттера. Эта команда была создана, поскольку это
    обычное действие при использовании Doctrine.


Также можно создать все известные сущности (например, любой PHP класс с
информацией для отображения Doctrine) для пакета или целого пространства имён:
...
Теперь есть удобный для использования класс ``Product`` с информацией для отображения, так что
Doctrine точно знает, как его сохранить. Конечно, пока нет соотвествующей таблицы
...
.. tip::
    ...
    Вообще-то, это необычайно мощная команда. Она сравнивает как *должна* выглядеть база
...
``product``, всё готово для сохранения данных в базу. Внутри контроллера это
очень просто. Добавьте следующий метод в ``DefaultController`` пакета:
...
* **строки 9-12** В этой части, берётся экземпляр объекта ``$product`` и с ним
  проводится работа, как и с любым другим, нормальным PHP объектом;

* **строка 14** Эта строка получает Doctrine-овый объект *entity manager*,
  отвественный за управление процессами сохранения объектов в базу данных и получения 
  их из базы данных;

* **строка 15** Метод ``persist()`` сообщает Doctrine команду на "управление"
  объектом ``$product``. Она не вызывает создание запроса к базе данных (пока).

* **строка 16** Когда вызывается метод ``flush()``, Doctrine просматривает все
...
.. note::
  ...
  вызывается метод ``flush()``, она просчитывает общий набор изменений и
  ...
  потому что быстр и эффективен.

При создании или обновлении объектов рабочий процесс всегда одинаков. В
следующем разделе вы увидите, что Doctrine достаточно умна для того, чтобы автоматически
выдать запрос ``UPDATE``, если запись уже существует в базе данных.

.. tip::

    Doctrine предоставляет библиотеку, позволяющую программно загружать тестовые
...
.. tip::
    ...
    Вы можете достигнуть того же результата без написания кода, испольуя ускоренный
    метод ``@ParamConverter``. Более детально см. 
...
.. note::
    ...
    использовать в Doctrine вместо полного имени класса для сущности (т.е.,
    ``Acme\StoreBundle\Entity\Product``). Оно будет работать, пока сущность
    находится в простанстве имён ``Entity`` вашего пакета.
...
для лёгкого извлечения объектов, основываясь на множественных условиях::
...
.. tip::
    ...
    нижнем правом углу на панели инструментов отладки web (web debug).
    ...
    Если кликнуть на иконке, откроется профилировщик, показывающий в точности, 
...
.. caution::
    ...
    Метод ``getSingleResult()`` генерирует исключение
    ...
    в блок попытка-перехват (try-catch block) и убедиться в том, что возвращается только один результат
...
позже), группировать и т. д. Дополнительная информация в документации Doctrine
`Doctrine Query Language`_.

.. sidebar:: Настройка параметров

    Обратите внимание на метод ``setParameter()``. Работая с Doctrine, хорошим тоном
    ...
    Позже можно указать значение placeholder-а ``price``  через метод
    ``setParameter()``::
    ...
    *всегда*. При использовании нескольких параметров, можно указать их значения за один
...
объектно-ориентированный интерфейс. Если используется IDE, то можно также получить
...
Персонализированные репозиторные классы (Custom Repository Classes) 
~~~~~~~~~~~~~~~~~~~~~~~~~
...
их, хорошим тоном будет создать песонализиованный репозиторный класс (custom 
repository class для сущности и добавить в него методы с запросами.

Чтобы сделать это, добавьте имя репозиторного класса в определение отображения 
(mapping definition).
...
.. note::
    ...
    Когда используется персонализированный репозиторный класс, всё ещё есть доступ к таким
...
``Product``. Начнём с создания сущности ``Category``. Так как известно, что в
...
``Product``. Далее, это делается не потому, что это нужно Doctrine, а потому,
что для приложения разумнее для каждого объекта ``Category`` иметь массив с 
объектами ``Product``.
...
.. tip::
   ...
   Значение targetEntity в выше использованном декораторе (decorator) ставит корректное 
   пространство имен в соответстие любой сущности, а не только сущностям, определенным
   в одном и том же пространстве имен. Чтобы содзать связь с сущностью , определенной в
   другом классе или пакете, введите полное пространство имен как targetEntity.
...
и ``Product`` с фактической связью один-ко-многим (with a natural one-to-many 
relationship). Класс ``Category`` имеет массив объектов ``Product`` и объект 
``Product`` может иметь один объект ``Category``. Другими словами - классы построены
таким способом, который имеет смысл для вашей задачи. А тот факт, что данные 
должны быть сохранены в базу данных, всегда второстепенен.
...
.. note::
    ...
    Эта задача, вообще-то, должна выполняться только во время разработки. Более надёжный
...
Когда необходимо получить объединённые объекты, рабочий процесс выглядит так же,
...
.. sidebar:: Связи и proxy классы
    ...
    Эта "ленивая загрузка" возможна потому, что, когда необходимо,  Doctrine
    ...
    пример, приведённый ранее::
        ...
        // выводит "Proxies\AcmeStoreBundleEntityCategoryProxy"
    ...
    на самом деле является proxy объектом, важно знать об этом.
...
.. note::
    ...
    Если использовать аннотации, необходимо предварять все аннотации упоминаниями об
...
большей части её опций. Чтобы узнать больше о настройке Doctrine, см. раздел
о Doctrine в :doc:`reference manual</reference/configuration/doctrine>`.
...
.. note::
    ...
    Предыдущие примеры предполагают, что свойство ``createdAt`` уже создано и
    отображено (здесь это не было показано).

Сразу же перед первым сохранением сущности, Doctrine автоматически вызовет этот
метод и в поле ``createdAt`` будет установлена текущая дата.
...
.. sidebar:: Lifecycle Callbacks и Приемники (Получатели) Событий (Event Listeners)
    ...
    email - необходимо зарегистрировать внешний класс как получатель (приемник) 
    событий (event listener или subscriber) и дать ему доступ к необходимым ресурсам. Дополнительную
...
* **Другие типы**
  ...
  * ``blob`` (отображается на resource stream)
  * ``simple_array`` (сериализуется, используя :phpfunction:`implode()` и :phpfunction:`explode()`,
    с запятой в качестве разделителя данных, и хранится в поле ``CLOB``)
  * ``json_array`` (сериализуется, используя :phpfunction:`json_encode()` и :phpfunction:`json_decode()`,
    и хранится в поле ``CLOB``)
  * ``guid``
...
.. note::
   ...
   необходимо установить пакет ``DoctrineFixturesBundle``. Чтобы узнать, как это
   сделать, прочтите статью ":doc:`/bundles/DoctrineFixturesBundle/index`" в
   документации.

.. tip::

    Эта страница показывает, как Doctrine работает внутри контроллера. Вы также можете 
    захотеть поработать с Doctrine где угодно в вашем приложении. Метод
    :method:`Symfony\\Bundle\\FrameworkBundle\\Controller\\Controller::getDoctrine`
    контоллера возвращает сервис ``doctrine``, и вы можете использовать его таким же
    образом где угодно в приложении путем инъекции в ваши собственные сервисы. См.
    :doc:`/book/service_container` за дополнительной информацией о создании своих собственных
    сервисов. 
...
время жизненного цикла сохранения.
