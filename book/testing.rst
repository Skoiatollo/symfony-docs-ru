...
.. note::
    ...
    Symfony2 работает с PHPUnit 3.5.11 или старше, впрочем, для тестирования 
    непосредственного основного (core code) Symfony вам понадобится версия 3.6.4.

Каждый тест - вне зависимости от того, функциональный он или модульный - это
PHP класс, который должен быть расположен в поддиректории `Tests/` ваших пакетов. Если вы
...
хотите тестировать поведение вашего приложения целиком, обратитесь к разделу
...
``Tests/Utility`` вашего пакета::
    ...
    class CalculatorTest extends \PHPUnit_Framework_TestCase
    {
        public function testAdd()
        {
            ...
            // подтвердите, что ваш калькулятор сложил числа правильно! 
...
.. note::
    ...
    вашего пакета. Таким образом, если вы тестируете класс вашего пакета из директории
...
.. code-block:: bash
    ...
    #запустить все тесты в директории Utility 
    $ phpunit -c app src/Acme/DemoBundle/Tests/Utility/

    # запустить все тесты для класса Calculator 
...
вы будете использовать для обхода вашего сайта::
...
.. tip::
    ...
    это XML или HTML документ. Неформатированный контент Response можно получить
    через вызов ``$client->getResponse()->getContent()``.

Давайте кликнем по ссылке, выбрав её при помощи Crawler используя выражение XPath
...
.. tip::
    ...
    полей (например, ``select()`` и ``tick()``). Подробнее читайте в разделе `Формы`_
    ниже.

Теперь, когда вы с лёгкостью можете перемещаться по приложению, воспользуйтесь
утверждениями, чтобы проверить, что оно действительно делает то, что вы от нее ожидаете. 
...
.. sidebar:: Подробнее о методе ``request()``:
    ...
    Массив ``server`` - это неотформатированные (raw) значения, которые вы,
    как правило, ожидаете найти в суперглобальном массиве `$_SERVER`_. Например, 
    для того, чтобы установить HTTP заголовки `Content-Type`, `Referer` и
    ``X-Requested-With``, вы должны передать следующее (учитывая префикс
     ``HTTP_`` для нестандартных заголовков)::
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    .. versionadded:: 2.4
        В Symfony 2.4. была добавлена поддержка для констант статус кода HTTP.
...
Используйте Crawler для нахождения DOM-элементов в теле Response. После эти элементы могут
...
или для выполнения более сложных запросов::
    ...
    // отправить необработанную строку JSON в теле запроса
    $client->request(
        'POST',
        '/submit',
        array(),
...
.. versionadded:: 2.3
    В Symfony 2.3. добавились методы ``getInternalRequest()`` и ``getInternalResponse()``.


Когда клиент используется для тестирования приложения, может возникнуть необходимость
...
Вы можете активизировать профайлер Symfony для каждого запроса, и он будет собирать
данные о внутренней обработке запроса. Например, профайлер может быть использован
...
Для получения профайлера для последнего запроса выполните следующий код::
    ...
    // активизируйте профайлер для непосредственно последующего запроса 
...
Когда запрос возвращает ответ с перенаправлением, клиент не следует
ему автоматически . Вы можете изучить ответ и форсировать перенаправление
после этого с помощью метода ``followRedirect()``::

    $crawler = $client->followRedirect();

Если вы хотите, чтобы клиент автоматически следовал всем перенаправлениям,
это можно форсировать с помошью метода ``followRedirects()``::
...
Crawler может извлечь информацию из узлов::
   ...
   // Возвращает значение атрибута для первого узла
    ...
    // Возвращает массив аттрибутов для каждого элемента (_text returns the node value) 
...
можно кликать, содержащие этот текст в атрибуте ``alt``. Как и друние фильтрующие методы,
этот возращает еще один объект ``Crawler``.


Как только вы выделили ссылку, вы получаете доступ к особому объекту ``Link``,
содержащему полезные методы, специфичные для ссылок (такие, как ``getMethod()`` и
``getUri()``). Для клика по ссылке, используйте клиентский метод ``click()`` и
передайте ему объект ``Link``::
...
.. note::
   ...
   несколько кнопок; если используются API перемещения, то помните, что надо искать
   кнопку.
...
Как только  у вас  есть объект Crawler представляющий собой кнопку, вызовите метод ``form()``, 
чтобы получить экземпляр ``Form`` для формы, содержащей узел кнопки::
...
.. tip::
    ...
    Чтобы узнать, как целенаправленно выбрать недействительные значения для select/radio, смотрите
...
Client используемый функциональными тестами создает Kernel который работает в особом "тестовом" 
(``test`` environment) окружении. Так как Symfony загружает ``app/config/config_test.yml``
