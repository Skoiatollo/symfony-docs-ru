...
Маршрут состоит из двух основных частей: пути (``path``), с которым
...
в пути – это метасимвол (wildcard). Он означает, что URL ``/hello/Ryan``,
...
в контроллер, так что вы сможете использовать его значение, чтобы поприветствовать
пользователя.
...
.. code-block:: php
    ...
    class HelloController extends Controller
    {
        public function indexAction($name)
        {
            ...
            // вместо этого создаем шаблон PHP
            // return $this->render(
...
block, который вы могли определить в шаблоне ``index.html.twig``. Так как вы не определили блок
